% SYMPTOMS
%-------------

symptoms(hiatal_hernia, [sharp_abdominal_pain, sharp_chest_pain, nausea, vomiting, difficulty_in_swallowing, heartburn, vomiting_blood]).
symptoms(gallstone, [sharp_abdominal_pain, upper_abdominal_pain, nausea, vomiting, back_pain, burning_abdominal_pain, sharp_chest_pain]).
%Gastroesophageal reflux disease
symptoms(gerb, [sharp_abdominal_pain, heartburn, vomiting, difficulty_in_swallowing, nausea, sharp_chest_pain, cough]).
symptoms(esophageal_cancer, [fatigue, ache_all_over, sharp_abdominal_pain, vomiting, difficulty_in_swallowing, weakness, shortness_of_breath, loss_of_appetite]).
symptoms(stomach_cancer, [fatigue, vomiting, nausea, sharp_abdominal_pain, weakness,  burning_abdominal_pain, diarrhea, loss_of_appetite, heartburn, vomiting_blood, blood_in_stool, jaundice]).



symptoms(gastritis, [sharp_abdominal_pain,vomiting, nausea, vomiting_blood,burning_abdominal_pain,upper_abdominal_pain,diarrhea, sharp_chest_pain, fever, headache, heartburn, regurgitation]).
symptoms(pancreatic_cancer, [sharp_abdominal_pain, jaundice, stomach_bloating, nausea, fatigue, diarrhea, arm_swelling, elbow_swelling, vomiting_blood]).
symptoms(ulcerative_colitis, [diarrhea, sharp_abdominal_pain, blood_in_stool, nausea, vomiting, burning_abdominal_pain, rectal_bleeding, lower_abdominal_pain, melena, loss_of_appetite, swollen_lymph_nodes, discharge_in_stools]).
symptoms(crohn_disease, [sharp_abdominal_pain, diarrhea, vomiting, nausea, burning_abdominal_pain, blood_in_stool, rectal_bleeding, pain_of_the_anus, joint_aches, stomach_bloating, heartburn, regurgitation]).

symptoms(hepatitis_A, [fatigue, nausea, sharp_abdominal_pain, joint_aches, itching, jaundice, loss_of_appetite, dark_colored_urine, light_colored_stools]).
symptoms(hepatitis_B, [fatigue, nausea, sharp_abdominal_pain, joint_aches, itching, jaundice, loss_of_appetite, dark_colored_urine, light_colored_stools]).
symptoms(hepatitis_C, [fatigue, nausea, sharp_abdominal_pain, joint_aches, itching, jaundice, loss_of_appetite, dark_colored_urine, light_colored_stools]).
symptoms(drug_induced_hepatitis,[fatigue, nausea, sharp_abdominal_pain, joint_aches, itching, jaundice, loss_of_appetite, dark_colored_urine, light_colored_stools]).
symptoms(inherited_hepatitis,[fatigue, nausea, sharp_abdominal_pain, joint_aches, itching, jaundice, loss_of_appetite, dark_colored_urine, light_colored_stools]).
symptoms(autoimmune_hepatitis,[fatigue, nausea, sharp_abdominal_pain, joint_aches, itching, jaundice, loss_of_appetite, dark_colored_urine, light_colored_stools]).

symptoms(cirrhosis, [fatigue, nausea, abdominal_discomfort, abdominal_swelling, weakness, itching, jaundice, loss_of_appetite, loss_of_weight]).

%-----------------------------------------------------------------------------------------------------------------------
% PERSONAL SYMPTOMS
%--------------------

personal_symptoms(ana, [sharp_abdominal_pain, sharp_chest_pain, nausea, heartburn,upper_abdominal_pain]).
personal_symptoms(anaa, [nausea, itching, joint_aches]).
personal_symptoms(una, [lower_abdominal_pain, melena]).

personal_symptoms(marti, [sharp_chest_pain, nausea, vomiting, difficulty_in_swallowing]). %cir na zeludcu
personal_symptoms(ljilja, [fatigue, vomiting, nausea, sharp_abdominal_pain]). %rak zeludca



personal_symptoms(aca,[headache]).
personal_symptoms(neca,[difficulty_in_swallowing]).
personal_symptoms(masa,[diarrhea]).
personal_symptoms(peca,[difficulty_in_swallowing]).
personal_symptoms(zivka,[jaundice]).
personal_symptoms(sara,[discharge_in_stools]).
personal_symptoms(stevan_matovic,[fatigue, sharp_abdominal_pain]).
personal_symptoms(biljana_matovic,[fatigue, sharp_abdominal_pain]).
personal_symptoms(zorica,[jaundice]).
personal_symptoms(tijana,[jaundice]).
personal_symptoms(sara,[jaundice]).
personal_symptoms(mima,[jaundice]).
personal_symptoms(mirko,[jaundice]).

personal_symptoms(maska,[jaundice]).
personal_symptoms(masks,[jaundice]).
personal_symptoms(mirka,[fatigue]).
